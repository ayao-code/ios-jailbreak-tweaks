/**
 * JDAntiJBDetect - 京东越狱检测绕过（轻量版 beta1）
 *
 * 基于 TBAntiJBDetect 优化版，适配京东 APP：
 *   1. ObjC 方法：isJailBreak / checkJailBreak 等 → 返回 NO
 *   2. NSFileManager fileExistsAtPath: → 越狱路径返回 NO
 *   3. C 层 stat / access → 越狱路径返回 ENOENT
 *   4. _dyld_get_image_name → 过滤越狱 dylib 名
 *   5. sysctlbyname → 清理越狱相关系统信息
 *   6. fopen / open / openat → 越狱路径返回 NULL / ENOENT
 *   7. getenv → 清理越狱环境变量
 *   8. opendir / readdir → 过滤越狱目录内容
 *   9. canOpenURL → cydia:// 等返回 NO
 *
 * beta1: 日志关闭，零 CPU 开销，不耗电
 */
...
static void initJBPaths(void) {
    ...
    dispatch_once(&onceToken, ^{
        ...
        jbDylibKeywords = [NSSet setWithArray:@[
            ...
            @"libblackjack", @"libkrw", @"JDAntiJBDetect",
            @"TBAntiJBDetect", @"Choicy", @"Crane",
        ]];
    });
}
...
static BOOL isJBDylibPath(NSString *path) {
    ...
    // 隐藏自身
    if ([lower containsString:@"jdantijbdetect"] || [lower containsString:@"tbantijbdetect"]) return YES;
